...
const list = async (user) => {
  ...
  return await irrigationEventRepository.findMany({ where: { plotId: { in: plots.map((plot) => plot.id) } }, orderBy: { createdAt: "desc" } });
};

const byPlot = async (plotId, user) => {
  await plotService.get(plotId, user);
  return await irrigationEventRepository.findMany({ where: { plotId }, orderBy: { createdAt: "desc" } });
};

const byDevice = async (deviceId, user) => {
  await deviceService.get(deviceId, user);
  return await irrigationEventRepository.findMany({ where: { deviceId }, orderBy: { createdAt: "desc" } });
};
